...
var parse = require("react-docgen-typescript").withCustomConfig('./tsconfig.json').parse;
...
function getComponentData(paths, componentName) {
  var filepath = path.join(paths.components, componentName,
    getComponentFile(path.join(paths.components, componentName), componentName));
  var content = readFile(filepath);
  var parsedData = parse(filepath)
  var info = {};
...
function getExampleData(examplesPath, componentName) {
  ...
  return examples.map(function(file) {
    ...
    return {
      ...
      name: file.slice(0, path.extname(file).length * -1),
      description: info.description,
      code: content
    };
  });
...
function getComponentFile(filepath, component) {
  return fs.readdirSync(filepath).filter(function(file) {
    return (
      fs.statSync(path.join(filepath, file)).isFile() &&
      path.basename(file).slice(0, path.extname(file).length * -1) === component
    );
  })[0];
}
